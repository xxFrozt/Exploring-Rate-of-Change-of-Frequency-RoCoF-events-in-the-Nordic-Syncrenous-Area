...
def create_scatter_max_RoCoF_vs_Nadir_2015_2023(df,ax):
    ...
    ax_big.set_title(r'Max RoCoF vs Nadir 2015 - 2023',size= 20) 
    ax_big.set_xlabel(r'RoCoF $\varphi$ [mHz{$\cdot$}s$^{-1}$]',size = 20)
    ax_big.set_ylabel(r'Nadir $\beta$ [mHz]',size = 20)
    ax_big.legend(fontsize=15)
    ax_big.tick_params(axis='x', labelsize=21, pad=3, length=5)
    ax_big.tick_params(axis='y', labelsize=21, pad=3, length=5)
...
for i, year in enumerate(years):
    ...
    ax[i].set_xticks([150,300])
    ax[i].tick_params(axis='y', labelsize=20, pad=3, length=5)  # This line changes the y-axis label size
    ...
    if i < 6:
        ...
        ax[i].set_xlabel('')
    ax[i].set_title(f'{year}', y=0.62,x=0.27, fontsize=20)
